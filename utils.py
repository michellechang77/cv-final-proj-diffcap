...
def get_optimizer(config, parameters):
    ...
    if config.optim.optimizer == 'Adam':
        return optim.Adam(parameters, lr=float(config.optim.lr), weight_decay=config.optim.weight_decay,
                          betas=(config.optim.beta1, 0.999), amsgrad=config.optim.amsgrad,
                          eps=config.optim.eps)
    elif config.optim.optimizer == 'AdamW':
        return optim.AdamW(parameters, lr=float(config.optim.lr), weight_decay=config.optim.weight_decay,)
